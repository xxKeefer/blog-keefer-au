...
import { AnchorSkeleton } from './AnchorSkeleton'
...
const LevelIconMap = new Map<HeadingLevel, JSX.Element>([
  [
    'h1',
    <ArrowRight key={'h1'} size={24} weight="fill" className="min-w-[24px]" />,
  ],
  [
    'h2',
    <ArrowRight key={'h2'} size={24} weight="bold" className="min-w-[24px]" />,
  ],
  [
    'h3',
    <ArrowBendDownRight
...
export const SideAnchorLinks = ({
  ...
  if (!headings) return <AnchorSkeleton />
  ...
  return (
    <>
      ...
      <ul
        className={`${textColor} flex flex-1 flex-col gap-2 overflow-y-auto p-4`}
      >
        {headings?.map((link) => {
          ...
          return (
            <li
              key={link.id}
              className={`${link.id} flex scroll-m-2 flex-row justify-start gap-2 ${textColor} ${activeStyles} ${levelStyle}`}
            >
